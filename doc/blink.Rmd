...
LED-Streufen sind in zahlreichen unterschiedlichen Versionen erhältlich, die sich durch Leistung, Lichtfarbe, und Versorgungsspannung unterscheiden. Die benötigte Leistung im Regelfall die durch Microcontroller wie Arduino oder ESP am doigitalen Pins erreichbaren Werte, außerdem beträgt die Versorgungsspannung oft 12 oder 24V und nicht 3.3 oder 5V wie beim Mikrocontroller. Es sind deshalb Bauteile zur Steuerung etwas höherer Deistungen erforderlich.
...
Das Blink-Beispiel bringt zunächst die interne LED des Arduino zum blinken bringen, 
anschließend soll eine externe LED an PIN 13 angesteuert werden. Das kürzere Pin der LED wird mit dem Minuspol (Masse) verbunden. Der längere Pin (Pluspol) wird über den Vorwiderstand (hier 300 Ohm) an Pin 13 des Arduino angeschlossen.
...
den Millisekunden-Zähler der internen Quarzuhr des Arduino. Das ist zum einen genauer, andererseits
wired der Programmfluss nicht durch "delay" unterbrochen. 
